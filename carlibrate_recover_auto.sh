...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/calibrate_recover_auto_$(date +%Y%m%d_%H%M%S).log"

echo "[run] python carlibrate_recover_auto_ui.py --config $CONFIG_PATH $*"
echo "[log] $LOG_FILE"
python -u carlibrate_recover_auto_ui.py --config "$CONFIG_PATH" "$@" 2>&1 | tee -a "$LOG_FILE"
