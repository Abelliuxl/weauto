...
try:
    ...
    if exc.name == "_tkinter":
        raise SystemExit(
            "Missing tkinter. Run `./carlibrate_row_title.sh config.toml` "
            "or use `.venv312/bin/python carlibrate_row_title_ui.py --config config.toml`."
        ) from exc
    raise
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visual calibrator for row title OCR region."
    )
...
class RowTitleRegionUI:
    def __init__(self, image, box: dict, on_save) -> None:
        ...
        self.root = tk.Tk()
        self.root.title("Row Title 文本区域校准")
    ...
    def _redraw(self) -> None:
        ...
        self.canvas.create_text(x1 + 40, y1 + 14, text="TITLE", fill=color, tags=("overlay",))
...
def main() -> None:
    ...
    init_w = int(box_w * cfg.row_title_region.w)
    init_h = int(box_h * cfg.row_title_region.h)
    y_from_bottom = int(box_h * cfg.row_title_region.y)
    init = {
        "x": int(box_w * cfg.row_title_region.x),
    ...
    print(
        f"[start] row-title-calibrator row_box=({box_x},{box_y},{box_w},{box_h}) "
    ...
    def _save(b: dict, w: int, h: int) -> None:
        ...
        text = _upsert_region_section(text, "row_title_region", rx, ry, rw, rh)
        text = _upsert_top_level_key(text, "row_title_region_enabled", "true")
        config_path.write_text(text, encoding="utf-8")
        print(
            f"[saved] row_title_region=({rx:.4f},{ry:.4f},{rw:.4f},{rh:.4f}) "
            "row_title_region_enabled=true"
        )

    ui = RowTitleRegionUI(image=shot, box=init, on_save=_save)
    ui.run()
